...
class PHYS_ZWave_Sol(PHYS_ZWave_Ocelot):
    ...
    def PHY_ZWave_100kbps_912MHz_OQPSK(self, model, phy_name=None):
        phy = self._makePhy(model, model.profiles.Base, readable_name='PHY_ZWave_100kbps_912MHz_OQPSK for Sol',
                            phy_name=phy_name, locked=False)
...
#        phy.profile_outputs.MODEM_CTRL5_BBSS.override = 6
        phy.profile_outputs.FEFILT0_DIGIGAINCTRL_BBSS.override = 6
        phy.profile_outputs.FEFILT1_DIGIGAINCTRL_BBSS.override = 6
    ...
    def PHY_ZWave_100kbps_916MHz(self, model, phy_name=None):
        phy = self._makePhy(model, model.profiles.Base, readable_name='PHY_ZWave_100kbps_916MHz_viterbi for Sol',
                            phy_name=phy_name, locked=False)
        ...
        phy.profile_inputs.symbols_in_timing_window.value = 12
        phy.profile_inputs.syncword_0.value = long(0xF0)
        ...
        ###c.a.w. revert to values used during Design validation
        phy.profile_outputs.MODEM_SQEXT_SQSTG2TIMOUT.override = 304
        phy.profile_outputs.MODEM_SQEXT_SQSTG3TIMOUT.override = 32
        phy.profile_outputs.MODEM_AFCADJRX_AFCSCALEE.override = 2
        phy.profile_outputs.MODEM_AFCADJRX_AFCSCALEM.override = 15
    ...
    def PHY_ZWave_40kbps_908MHz(self, model, phy_name=None):
        phy = self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_908MHz', locked=False)
        self.ZWave_40kbps_908MHz_legacy_base(phy, model)

        return phy

    def PHY_ZWave_40kbps_908MHz_viterbi(self, model, phy_name=None):
        phy = self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_908MHz_viterbi', locked=False)
        ...
        # Timings based on Zwave_LR_timing_21.xlsx
        # Based on sequential scanning of R3, R2, R1, LR
        phy.profile_outputs.MODEM_SQ_SQTIMOUT.override = int(464 / 25) # Timeout before checking for timing detect. Units: T_b. Zwave_LR_timing_21.xlsx:I15
        phy.profile_outputs.MODEM_SQEXT_SQSTG2TIMOUT.override = 80 # Timeout after timing detect to check for preamble. Units T_b.
        phy.profile_outputs.MODEM_SQEXT_SQSTG3TIMOUT.override = 16 + 16 # Timeout after preamble to check for sync word. Units T_b. Cell B25
    ...
    def PHY_ZWave_40kbps_9p6kbps_908MHz_legacy_conc(self, model, phy_name=None):
        phy = self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_9p6kbps_908MHz_legacy_conc', locked=False)
        self.ZWave_40kbps_908MHz_legacy_base(phy, model)
        self.ZWave_9p6kbps_908MHz_bcr_conc_base(phy, model)
        phy.profile_outputs.MODEM_PHDMODCTRL_BCRLEGACYCONC.override = 1

        # Overrides for the SQ block
        phy.profile_outputs.MODEM_SQ_SQEN.override = 1
        phy.profile_outputs.MODEM_SQ_SQTIMOUT.override = int(1986 / 25) # Timeout before checking for timing detect. Units: T_b.
        phy.profile_outputs.MODEM_SQEXT_SQSTG2TIMOUT.override = 80 # Timeout after timing detect to check for preamble. Units T_b.
    ...
    #Unlocking these Ocelot PHYs

    def PHY_ZWave_40kbps_9p6kbps_908MHz_viterbi_conc(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_9p6kbps_908MHz_viterbi_conc', locked=False)
        phy = super().PHY_ZWave_40kbps_9p6kbps_908MHz_viterbi_conc(model)
        return phy
    def PHY_ZWave_9p6kbps_908MHz(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, 'PHY_ZWave_9p6kbps_908MHz', locked=False)
        phy = super().PHY_ZWave_9p6kbps_908MHz(model)
        return phy

    def PHY_ZWave_40kbps_908MHz(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_908MHz', locked=False)
        phy = super().PHY_ZWave_40kbps_908MHz(model)
        return phy

    def PHY_ZWave_40kbps_908MHz_viterbi(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, 'PHY_ZWave_40kbps_908MHz_viterbi', locked=False)
        phy = super().PHY_ZWave_40kbps_908MHz_viterbi(model)
        return phy

    def PHY_ZWave_100kbps_916MHz_viterbi(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, readable_name='PHY_ZWave_100kbps_916MHz_viterbi for Ocelot',
                            phy_name=phy_name, locked=False)
        phy = super().PHY_ZWave_100kbps_916MHz_viterbi(model)
        return phy

    def PHY_ZWave_100kbps_912MHz_OQPSK(self, model, phy_name=None):
        self._makePhy(model, model.profiles.Base, readable_name='PHY_ZWave_100kbps_912MHz_OQPSK for Ocelot',
                            phy_name=phy_name, locked=False)
        phy = super().PHY_ZWave_100kbps_912MHz_OQPSK(model)
        return phy

     #Not inheriting these PHYs
    ...
    def PHY_SigGen_ZWave_100kbps_912MHz_OQPSK(self, model, phy_name=None):
        pass