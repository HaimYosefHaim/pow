...
class calc_softmodem_sol(ICalculator):
    ...
    def calc_fcs_type_802154(self, model):
        ...
        if  'wisun' in model.profile.name.lower() or 'wi-sun' in  model.profile.name.lower():
    ...
    def calc_sun_oqpsk_chiprate(self, model):

        #Read in model variables
        chip_rate = model.vars.baudrate.value

        if chip_rate == 100000:
            sun_oqpsk_chiprate = model.vars.sun_oqpsk_chiprate.var_enum._100_KCPS
        elif chip_rate == 400000:
            sun_oqpsk_chiprate = model.vars.sun_oqpsk_chiprate.var_enum._400_KCPS
        elif chip_rate == 1000000:
            sun_oqpsk_chiprate = model.vars.sun_oqpsk_chiprate.var_enum._1000_KCPS
        else:
            sun_oqpsk_chiprate = model.vars.sun_oqpsk_chiprate.var_enum._2000_KCPS

        #Write the model variable
        model.vars.sun_oqpsk_chiprate.value = sun_oqpsk_chiprate
    ...
    def calc_softmodem_cw(self, model):
        ...
        if softmodem_modulation_type == model.vars.softmodem_modulation_type.var_enum.CW:
            ...
            freq_field = (dac_freq_actual / 2) * (base_frequency_hz / (2**15)) # freq = (tx_sampling_freq / 2) * (freq / (2 ** 15))

            #Set digital amplitude to fullscale for now
            amp_field = 32767 # // [0:32767]      amplitude of the CW tone
        ...
        self._reg_write(model.vars.CW_CWCFG1_FREQ, int(freq_field), do_not_care=do_not_care)
    ...
    def calc_softmodem_legoqpsk(self, model):
        ...
        # Write the registers
        self._reg_write(model.vars.LEGOQPSK_LEGOQPSKCFG_MODULATION, int(modulation_field), do_not_care=do_not_care)
        self._reg_write(model.vars.LEGOQPSK_LEGOQPSKCFG_CHIPRATE, int(chiprate_field), do_not_care=do_not_care)
        self._reg_write(model.vars.LEGOQPSK_LEGOQPSKCFG_BANDFREQMHZ, int(bandfreqmhz_field), do_not_care=do_not_care)
    ...
    def calc_softmodem_sunfsk(self, model):
        ...
        self._reg_write(model.vars.SUNFSK_SUNFSKCFG1_OSR, osr_field, do_not_care=do_not_care)
