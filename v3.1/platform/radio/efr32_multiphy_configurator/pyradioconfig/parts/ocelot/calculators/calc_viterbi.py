...
class CALC_Viterbi_ocelot(CALC_Viterbi_lynx):
        ...
        if preamsch_len > 0 and vtdemoden:
            if ((freq_dev_min < 0.8*deviation) or (freq_dev_max > 1.2*deviation)):
                ...
                if antdivmode == model.vars.antdivmode.var_enum.PHDEMODANTDIV or antdivmode == model.vars.antdivmode.var_enum.ANTENNA1:
                    ...
                    reg = 80 #Optimized value because our calculation isn't yet good for small windows (window size is 8)
                else:
    ...
    def calc_vtbletimingsel_reg(self, model):
        ...
        if (demod_select == model.vars.demod_select.var_enum.TRECS_SLICER or demod_select == model.vars.demod_select.var_enum.TRECS_VITERBI) and not trecs_remoden:
            vtbletimingsel = 1
        else:
            vtbletimingsel = 0

        # Write the register
        self._reg_write(model.vars.MODEM_VTBLETIMING_VTBLETIMINGSEL, vtbletimingsel)
