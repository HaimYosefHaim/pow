...
static psa_key_handle_t master_key_id;

/// HKDF key identifier
static psa_key_handle_t hkdf_key_id;
  ...
  switch (app_state) {
    ...
    case SELECT_KEY_SIZE:
      ...
      if (enter_press) {
        ...
        if ((derive_process == PSA_ALG_CHACHA20)
    ...
    case DERIVE_HKDF_KEY:
      ...
      if (derive_key(CREATE_KEY_OFFSET + symmetric_key_storage_select,
                     get_kdf_op_ptr(),
                     symmetric_key_size[symmetric_key_size_select],
                     DERIVE_KEY_ID,
