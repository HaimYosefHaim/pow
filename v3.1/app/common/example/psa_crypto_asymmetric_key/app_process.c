...
static psa_key_handle_t source_key_id;

/// Copied key identifier
static psa_key_handle_t copy_key_id;
...
void app_process_action(void)
{
  ...
  switch (app_state) {
    ...
    case CREATE_KEY:
      if (selected_key_size == 448) {
        printf("  + The x448 ECC curve is not supported yet\n");
        print_key_storage();
        break;
      }
    ...
    case IMPORT_PRIVATE_KEY:
      ...
      memcpy(get_key_buf_ptr(), get_test_key_buf_ptr(true),
             PSA_KEY_EXPORT_MAX_SIZE(PSA_KEY_TYPE_ECC_KEY_PAIR(asymmetric_key_curve[asymmetric_key_curve_select]),
                                     selected_key_size));
      set_key_len(PSA_KEY_EXPORT_MAX_SIZE(PSA_KEY_TYPE_ECC_KEY_PAIR(asymmetric_key_curve[asymmetric_key_curve_select]),
                                          selected_key_size));
    ...
    case COPY_KEY:
      ...
      if ((asymmetric_key_storage_select ^ 0x01) == VOLATILE_PLAIN_KEY) {
        ret = copy_key(0, 0, ASYM_KEY_ALGO);
      } else {
        ret = copy_key(PERSISTENT_COPY_KEY_ID, 0, ASYM_KEY_ALGO);
      }
      if (ret == PSA_SUCCESS) {
    ...
    case IMPORT_PUBLIC_KEY:
      ...
      if (asymmetric_key_curve_select == 0) {
        memcpy(get_key_buf_ptr(), get_test_key_buf_ptr(false),
               PSA_KEY_EXPORT_MAX_SIZE(PSA_KEY_TYPE_ECC_PUBLIC_KEY(asymmetric_key_curve[asymmetric_key_curve_select]),
                                       selected_key_size));
        set_key_len(PSA_KEY_EXPORT_MAX_SIZE(PSA_KEY_TYPE_ECC_PUBLIC_KEY(asymmetric_key_curve[asymmetric_key_curve_select]),
                                            selected_key_size));
      } else {
        // PSA_KEY_EXPORT_MAX_SIZE() doesn't support Montgomery public key yet
