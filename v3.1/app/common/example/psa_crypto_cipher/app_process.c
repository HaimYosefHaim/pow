...
void app_process_action(void)
{
  ...
  switch (app_state) {
    ...
    case CHACHA20_TEST:
      // CHACHA20 with wrapped key is not supported yet
      ...
      if (symmetric_key_size_select != KEY_SIZE_MAX) {
        ...
        printf("\n  . CHACHA20 encryption -");
        encrypt_cipher_single(PSA_ALG_CHACHA20);
        if (app_state == PSA_CRYPTO_EXIT) {
          break;
        }
        printf("\n  . CHACHA20 decryption -");
        decrypt_cipher_single(PSA_ALG_CHACHA20);
        if (app_state == PSA_CRYPTO_EXIT) {
          break;
        }
      }
    ...
    case OPEN_BUILTIN_KEY:
      ...
      printf("  + Opening the AES-128 key in the SE OTP... ");
      if (open_builtin_key(SL_SE_BUILTIN_KEY_AES128_ID) == PSA_SUCCESS) {
    ...
    case BUILTIN_KEY_ENCRYPTION:
      ...
      if (update_cipher_encryption(0) == PSA_ERROR_DOES_NOT_EXIST) {
    ...
    case CLOSE_BUILTIN_KEY:
      app_state = PSA_CRYPTO_EXIT;
      printf("  + Closing the AES-128 key in the SE OTP... ");
...
static void encrypt_cipher_single(psa_algorithm_t algo)
{
  ...
  memset(get_cipher_msg_buf_ptr(), 0,
         PSA_BLOCK_CIPHER_BLOCK_SIZE(PSA_KEY_TYPE_AES) + plain_msg_size[plain_msg_size_select]);
