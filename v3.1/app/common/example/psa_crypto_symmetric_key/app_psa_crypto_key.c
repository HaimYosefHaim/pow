...
/// Key identifier
static psa_key_handle_t key_id;
...
{
  print_error_cycle(psa_open_key(builtin_key_id, &key_id));
}
...
{
  print_error_cycle(psa_close_key(key_id));
}
                               ...
                               psa_algorithm_t algo)
{
  ...
  if (PSA_ALG_IS_RANDOMIZED_ECDSA(algo)) {
    // ECDSA
  ...
  } else if (algo == PSA_ALG_CHACHA20 || algo == PSA_ALG_CHACHA20_POLY1305) {
...
{
  psa_key_handle_t temp_id = key_id;
