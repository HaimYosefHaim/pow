...
/// EFR32xG21A user configuration
static const uint32_t xg21a_conf[XG21_USER_CONF_SIZE] = {
...
/// EFR32xG21B user configuration
static const uint32_t xg21b_conf[XG21_USER_CONF_SIZE] = {
  0x00000018,                                   // 24 bytes data below
  SECURE_BOOT_ENABLE_MASK + ANTI_ROLLBACK_MASK, // MCU settings
...
/// EFR32xG22 user configuration
static const uint32_t xg22_conf[XG22_USER_CONF_SIZE] = {
...
void set_dci_command(uint32_t *cmd_buf)
{
  ...
  switch (*cmd_buf) {
    ...
    case INIT_HSE_OTP_XG21A:
      *cmd_buf = INIT_SE_OTP_LENGTH;
      *(++cmd_buf) = COMMAND_INIT_OTP;
      // Calculate parity of OTP configuration
      *(++cmd_buf) = 0;
      for (i = 1; i < XG21_USER_CONF_SIZE; i++) {
        *cmd_buf ^= xg21a_conf[i];
      }
      // Copy OTP configuration to buffer
      memcpy((uint32_t *)(++cmd_buf), (uint32_t *)xg21a_conf,
             INIT_SE_OTP_LENGTH);
      break;
    ...
    case INIT_HSE_OTP_XG21B:
      ...
      *(++cmd_buf) = 0;
      for (i = 1; i < XG21_USER_CONF_SIZE; i++) {
        *cmd_buf ^= xg21b_conf[i];
      }
      // Copy OTP configuration to buffer
      memcpy((uint32_t *)(++cmd_buf), (uint32_t *)xg21b_conf,
             INIT_SE_OTP_LENGTH);
      break;
    ...
    case INIT_VSE_OTP_XG22:
      ...
      *(++cmd_buf) = xg22_conf[1];
      // Copy OTP configuration to buffer
      memcpy((uint32_t *)(++cmd_buf), (uint32_t *)xg22_conf,
             INIT_VSE_OTP_LENGTH);
      break;
