...
void get_device_info(char *name, uint64_t *uid)
{
  ...
  *uid |= read_mem((uint32_t)&(DEVINFO->EUI64L));
  ...
  // Enable MSC clock if device is EFR32xG22
  buf0 &= _DEVINFO_PART_FAMILYNUM_MASK;
  if (buf0 == XG22_FAMILY) {
    write_mem((uint32_t)(CLKEN1_SET_ADDR), CLKEN1_SET_MSC);
    // Check UDLOCKBIT on EFR32xG22
...
void verify_user_data(uint32_t *ptr)
{
  ...
  if (ptr == NULL) {
    ...
    addr = USER_DATA_ADDR;
    while (addr < (USER_DATA_ADDR + user_data_size)) {
    ...
    verify_fw_image(USER_DATA_ADDR, user_data_size, (uint32_t *)ptr);
  }
}
...
uint32_t erase_flash(bool userdata)
{
  ...
  if (userdata) {
    erase_flash_page(USER_DATA_ADDR, user_data_size);
    addr = USER_DATA_ADDR;
    while (addr < (USER_DATA_ADDR + user_data_size)) {
      if (read_mem(addr) != 0xFFFFFFFF) {
        RAISE(SWD_ERROR_DEVICE_ERASE_FAILED);
...
static void erase_flash_page(uint32_t start_addr, uint32_t size)
{
  ...
  if (start_addr != 0) {
    ...
    // Start from address 0, unlock mass erase
