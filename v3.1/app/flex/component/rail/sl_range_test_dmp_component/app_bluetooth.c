 ...
 ******************************************************************************/
static void remove_last_indication(uint8_t characteristic);
 ...
 ******************************************************************************/
static bool is_indication_enabled(uint8_t characteristic);

/*******************************************************************************
 * Update the setting of the indication of the selected characteristic
 ******************************************************************************/
static void update_indication_enabled(uint8_t characteristic, bool enabled);
...
static bd_addr bluetooth_address;
...
void sl_bt_on_event(sl_bt_msg_t* evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      APP_INFO("[info] [B] Booted: v%d.%d.%d-b%d\n",
      ...
      APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_system_get_identity_address failed with %#X\n", bt_status);

      APP_INFO("[info] [B] Bluetooth %s address: %02X:%02X:%02X:%02X:%02X:%02X\n",
      ...
      APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_advertiser_create_set failed with %#X\n", bt_status);
      ...
      APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_write_attribute_value failed with %#X\n", bt_status);
    ...
    case sl_bt_evt_connection_opened_id:
      ...
      APP_INFO("[info] [B] Connection opened\n");
    ...
    case sl_bt_evt_connection_closed_id:
      connection_handler = 0xFF;
      APP_INFO("[info] [B] Connection closed\n");
      ...
      if (gattdb_pktsReq == evt->data.evt_gatt_server_user_write_request.characteristic) {
        ...
        if ( value < number_of_phys ) {
          ...
          bt_status = sl_bt_gatt_server_send_user_write_response(
            ...
            gattdb_phy,
            0);
          APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_user_write_response failed with %#X\n", bt_status);
          update_needed = true;
          apply_new_phy(false);
          update_phy_channel_range();
          update_phy_payload_range();
        ...
        APP_INFO("[warning] [B] Unhandled characteristic write!\n");
      }

      APP_INFO("[info] [B] GATT write: %u\n", evt->data.evt_gatt_server_user_read_request.characteristic);
    ...
    case sl_bt_evt_gatt_server_user_read_request_id:
      if (gattdb_PER == evt->data.evt_gatt_server_user_read_request.characteristic) {
        ...
        APP_INFO("[warning] [B] Unhandled characteristic read!\n");
      }
      APP_INFO("[info] [B] GATT read: %u\n", evt->data.evt_gatt_server_user_read_request.characteristic);
      break;
...
void deactivate_bluetooth(void)
{
  ...
  if (connection_handler != 0xFF) {
    ...
    APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_connection_close failed with %#X\n", bt_status);
...
void advertise_received_data(int8_t rssi, uint16_t packet_count, uint16_t received_packets)
{
  ...
  APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_advertiser_set_data failed with %#X\n", bt_status);
  ...
  APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_advertiser_set_timing failed with %#X\n", bt_status);
  ...
  APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_advertiser_start failed with %#X\n", bt_status);
 ...
 * @return None
 ******************************************************************************/
void add_bluetooth_indication(uint8_t characteristic)
{
  if (connection_handler != 0xFF) {
    if (is_indication_enabled(characteristic)) {
      if (range_test_indicatons.count_of_indications < 16) {
        ...
        APP_INFO("Indication queue is full\n");
      }
    }
  }
}
    ...
    if (!indication_is_under_way) {
      if (range_test_indicatons.count_of_indications > 0) {
        ...
        switch (characteristic) {
          ...
          {
            uint16_t per = (uint16_t)(range_test_measurement.PER * 10);
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&per, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
          ...
          {
            uint16_t moving_average = (uint16_t)(range_test_measurement.moving_average * 10);
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&moving_average, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
          }
          break;
          case gattdb_pktsSent:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_measurement.packets_sent, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_pktsCnt:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_measurement.packets_received_counter, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_pktsRcvd:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_measurement.packets_received_correctly, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_pktsReq:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_settings.packets_repeat_number, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_channel:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_settings.channel, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_phy:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.current_phy, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            update_phy_channel_range();
            update_phy_payload_range();
          ...
          case gattdb_radioMode:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.radio_mode, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_txPower:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 2, (uint8_t*)&range_test_settings.tx_power, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_destID:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.destination_id, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_srcID:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.source_id, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_payload:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.payload_length, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_maSize:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.moving_average_window_size, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_log:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_settings.usart_log_enable, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          case gattdb_isRunning:
            bt_status = sl_bt_gatt_server_send_characteristic_notification(connection_handler, characteristic, 1, (uint8_t*)&range_test_measurement.tx_is_running, &sent_length);
            APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_characteristic_notification failed with 0x%04X\n", bt_status);
            indication_is_under_way = true;
            break;
          default:
...
static void start_advertising(void)
{
  ...
  APP_INFO("[info] [B] Start advertising\n");
}
...
static void stop_advertising(void)
{
  ...
  APP_ASSERT(bt_status == SL_STATUS_OK, "sl_bt_advertiser_stop failed with %#X\n", bt_status);
  APP_INFO("[info] [B] Stop advertising\n");
...
static bool answer_read_request(uint8_t* answer_value, uint8_t value_length, sl_bt_msg_t *evt)
{
  ...
  bt_status = sl_bt_gatt_server_send_user_read_response(
    ...
    answer_value,
    &sent_len);
  APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_user_read_response failed with 0x%04X\n", bt_status);
...
static bool check_and_write_uint8_value(uint8_t *update_value_byte, uint16_t range_attribute, size_t range_max_size, sl_bt_msg_t *evt)
{
  ...
  APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_read_attribute_value failed with 0x%04X\n", bt_status);
  ...
  if ((min_range <= value) && (value <= max_range)) {
    *update_value_byte = value;
    bt_status = sl_bt_gatt_server_send_user_write_response(
      ...
      0);
    APP_WARNING(bt_status == SL_STATUS_OK, "sl_bt_gatt_server_send_user_write_response failed with 0x%04X\n", bt_status);
    success = true;
  } else {
    bt_status = sl_bt_gatt_server_send_user_write_response(
      evt->data.evt_gatt_server_user_write_request.connection,
...
static void remove_last_indication(uint8_t characteristic)
{
  if (range_test_indicatons.count_of_indications > 0) {
    if (characteristic == range_test_indicatons.characteristic[0]) {
      ...
      APP_INFO("Not correct indication\n");
    }
  }
  if (range_test_indicatons.count_of_indications > 0) {
    RTOS_ERR err;
