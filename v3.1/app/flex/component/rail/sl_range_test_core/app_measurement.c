...
#define PRINT_AND_CLEAR_FLAG(flag)  if (flag) { flag = 0; APP_INFO(" %s\n", GET_VARIABLE_NAME(flag)); }
...
int16_t get_min_tx_power_deci_dbm(void)
{
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "get_min_tx_power_deci_dbm:RAIL_GetTxPowerConfig failed with %d\n", status);
  tx_power_mode = tx_power_config.mode;
  tx_power_curve_ptr = RAIL_GetTxPowerCurve(tx_power_mode);
  APP_WARNING(tx_power_curve_ptr != NULL, "get_min_tx_power_deci_dbm:RAIL_GetTxPowerCurve failed with NULL pointer.\n");
...
int16_t get_max_tx_power_deci_dbm(void)
{
  ...
  rail_handle = get_current_rail_handler();
  status = RAIL_GetTxPowerConfig(rail_handle, &tx_power_config);
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "get_max_tx_power_deci_dbm:RAIL_GetTxPowerConfig failed with %d\n", status);
  tx_power_mode = tx_power_config.mode;
  tx_power_curve_ptr = RAIL_GetTxPowerCurve(tx_power_mode);
  APP_WARNING(tx_power_curve_ptr != NULL, "get_max_tx_power_deci_dbm:RAIL_GetTxPowerCurve failed with NULL pointer.\n");
  if (tx_power_curve_ptr == NULL) {
    int16_t maxPower;
    maxPower = 200;
...
void update_tx_power(void)
{
  ...
  APP_WARNING(power_status == RAIL_STATUS_NO_ERROR, "RAIL_SetTxPowerDbm failed with %d\n", power_status);
}
...
#endif
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_StartRx failed with code %d", status);
...
bool receive_measurment(void)
{
  ...
  if (rx_crc_error_happpend) {
    ...
    APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_StartRx failed with %d", rail_status);
#else
  ...
  if (rx_packet_received > 0) {
    ...
    if (is_current_phy_standard()) {
      ...
      rx_packet = (range_test_packet_t*) rx_fifo;
    }
...
static void send_packet(uint16_t packet_number)
{
  ...
  uint8_t send_buffer[RAIL_TX_BUFFER_SIZE];
  ...
  RAIL_WriteTxFifo(rail_handle, send_buffer, tx_length, true);
  ...
  if (!set_tx_failed) {
    ...
    if (rail_status != RAIL_STATUS_NO_ERROR) {
      APP_INFO("RAIL_StartTx() error 0x%0X\n", rail_status);
    }
  }
}
...
static inline void print_tx_logs(void)
{
  ...
  APP_INFO("Sent, Actual:%u, Max:%u, IdS:%u, IdR:%u\n",
           packets_sent,
