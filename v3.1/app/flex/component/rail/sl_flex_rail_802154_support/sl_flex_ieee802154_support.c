                                              ...
                                              uint16_t packet_size)
{
  ...
  if (sl_flex_ieee802154_get_std() == SL_FLEX_IEEE802154_STD_IEEE802154_2P4GHZ) {
    ...
    status = RAIL_StartCcaCsmaTx(rail_handle,
                                 ...
                                 &csma_config_2p4,
                                 NULL);
    APP_WARNING(status == RAIL_STATUS_NO_ERROR,
                "RAIL_StartCcaCsmaTx status: %d failed", status);
  } else {
    // starts the TX on the desired channel with the tx option and CSMA
    // functionality
    status = RAIL_StartCcaCsmaTx(rail_handle,
                                           ...
                                           uint8_t *frame_buffer)
{
  ...
  if ((frame == NULL) || (frame == NULL) || (frame_buffer == NULL)) {
    APP_WARNING(false,
                "sl_flex_ieee802154_pack_data_frame ERR: parameter");
                                             ...
                                             uint16_t max_frame_size)
{
  ...
  if ((frame == NULL) || (frame_buffer == NULL)) {
    APP_WARNING(false,
                "sl_flex_ieee802154_unpack_data_frame ERR: parameter");
    return SL_FLEX_IEEE802154_ERROR;
  }
  ...
  switch (std) {
    case SL_FLEX_IEEE802154_STD_IEEE802154_2P4GHZ:
      ...
      if (frame->payload == NULL) {
        APP_WARNING(false,
                    "sl_flex_802154_packet_unpack_data_frame failed");
        return SL_FLEX_IEEE802154_ERROR;
      }
      ...
      if (frame->payload_size > max_payload_size) {
        APP_WARNING(false,
                    "sl_flex_802154_packet_unpack_data_frame failed: Corrupted frame received: payload is too large.");
        return SL_FLEX_IEEE802154_ERROR;
      }
                                         ...
                                         sl_flex_ieee802154_std_t std)
{
  ...
  switch (std) {
    case SL_FLEX_IEEE802154_STD_IEEE802154_2P4GHZ:
      ...
      APP_WARNING(status == RAIL_STATUS_NO_ERROR,
                  "sl_rail_util_protocol_config status: %d failed", status);
  ...
  if ((std == SL_FLEX_IEEE802154_STD_IEEE802154G_863MHZ)
    ...
    status = RAIL_IEEE802154_ConfigGOptions(r_handle,
                                            ...
                                            RAIL_IEEE802154_G_OPTION_GB868);
    APP_WARNING(status == RAIL_STATUS_NO_ERROR,
                "RAIL_IEEE802154_ConfigGOptions status: %d failed", status);
    if (status != RAIL_STATUS_NO_ERROR) {
      return SL_FLEX_IEEE802154_ERROR;
    }
  }
                                                    ...
                                                    uint16_t address)
{
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR,
              "app_ieee802154_set_pan_id_filter() status: %d failed", status);
  if (status != RAIL_STATUS_NO_ERROR) {
    return SL_FLEX_IEEE802154_ERROR;
  }
                                                        ...
                                                        uint16_t address)
{
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR,
              "RAIL_IEEE802154_SetShortAddress() status: %d failed", status);
  if (status != RAIL_STATUS_NO_ERROR) {
    return SL_FLEX_IEEE802154_ERROR;
  }
                                                       ...
                                                       uint64_t *address)
{
  ...
  status = RAIL_IEEE802154_SetLongAddress(r_handle, (uint8_t*)address,
                                          ADDRESS_FILTER_INDEX);
  APP_WARNING(status == RAIL_STATUS_NO_ERROR,
              "RAIL_IEEE802154_SetLongAddress() status: %d failed", status);
  if (status != RAIL_STATUS_NO_ERROR) {
    return SL_FLEX_IEEE802154_ERROR;
  }
