...
void sl_simple_rail_rx_tick(void)
{
  ...
  if (rx_state_machine_update_failed) {
    APP_WARNING(false, "Simple RAIL Rx state machine update failed in sl_simple_rail_rx_on_event");
  ...
  switch (sl_simple_rail_rx_get_state()) {
    case S_RX_PACKET_RECEIVED:
      ...
      APP_WARNING(simple_rail_rx_status == SL_STATUS_OK, "Simple RAIL Rx state machine update failed");
      break;
    case S_RX_NOT_VALID_PACKET_RECEIVED:
      APP_WARNING(false, "No such RAIL Rx packet yet exists or RAIL handle is not active");
      simple_rail_rx_status = simple_rail_rx_set_state(S_RX_IDLE);
      APP_WARNING(simple_rail_rx_status == SL_STATUS_OK, "Simple RAIL Rx state machine update failed");
      break;
    case S_RX_PACKET_ERROR:
      // Handle Rx error
      APP_WARNING(false, "Radio RX Error occured\nEvents: %lld\n", current_rail_rx_err);
      simple_rail_rx_status = simple_rail_rx_set_state(S_RX_IDLE);
      APP_WARNING(simple_rail_rx_status == SL_STATUS_OK, "Simple RAIL Rx state machine update failed");
      break;
    default:
