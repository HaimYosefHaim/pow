...
void init_ranget_test_standard_phys(uint8_t* number_of_phys)
{
  ...
  *number_of_phys = *number_of_phys + NUM_OF_PREDEFINED_PHYS;
  for (int i = 0; i < PROT_NO_OF_ELEMENTS; i++) {
    ...
    APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_ConfigEvents failed");
    ...
    if (PROT_IEEE802154 == i) {
      ...
      APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_IEEE802154_Init failed");
      ...
      APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_IEEE802154_Config2p4GHzRadio failed");
      ...
      APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_BLE_ConfigPhy1MbpsViterbi faild with %d\n", status);
      ...
      status = RAIL_BLE_ConfigChannelRadioParams(rail_handles[PROT_BLE],
                                                 ...
                                                 BLE_LOGICAL_CH,
                                                 DISABLE_WHITENING);
      APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_BLE_ConfigChannelRadioParams failed");
      RAIL_Idle(rail_handles[PROT_BLE], RAIL_IDLE, true);
    }
  ...
  switch (current_phy_standard_value()) {
    ...
    case BLE_125KBPS:
      if (range_test_std_phys[current_phy_standard_value()].is_supported) {
        ...
        APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_BLE_Coding_125kbps faild with %d\n", status);
      }
      break;
    case BLE_500KBPS:
      if (range_test_std_phys[current_phy_standard_value()].is_supported) {
        ...
        APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_BLE_Coding_500kbps faild with %d\n", status);
      }
      break;
#endif
#if RAIL_BLE_SUPPORTS_1MBPS
    ...
    case BLE_2MBPS:
      if (range_test_std_phys[current_phy_standard_value()].is_supported) {
        ...
        APP_WARNING(status == RAIL_STATUS_NO_ERROR, "RAIL_BLE_ConfigPhy2MbpsViterbi faild with %d\n", status);
      }
      break;
#endif
    default:
...
/*******************************************************************************
 * Set the correct value for the standared phy channels
 * @param None
 * @return None
 ******************************************************************************/
