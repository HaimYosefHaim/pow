...
void sl_simple_rail_tx_tick(void)
{
  ...
  if (tx_state_machine_update_failed) {
    APP_WARNING(false, "Simple RAIL Tx state machine update failed in sl_simple_rail_tx_on_event");
  ...
  switch (sl_simple_rail_tx_get_state()) {
    case S_TX_PACKET_SEND_REQUEST:
      ...
      APP_ASSERT(start_tx_status == SL_STATUS_OK,
                 "[E: 0x%04x]: RAIL_StartTx() failed\n",
                 (int)start_tx_status);
      simple_rail_tx_status = simple_rail_tx_set_state(S_TX_PACKET_SENDING);
      APP_WARNING(simple_rail_tx_status == SL_STATUS_OK, "Simple RAIL Tx state machine update failed");
      break;
    ...
    case S_TX_PACKET_ERROR:
      APP_WARNING(false, "Radio TX Error occured\nEvents: %lld\n", current_rail_tx_err);
                                            ...
                                            const uint8_t num_of_bytes)
{
  ...
  if (num_of_bytes <= TX_PAYLOAD_LENGTH) {
    ...
    // Too many bytes are tried to be sent
