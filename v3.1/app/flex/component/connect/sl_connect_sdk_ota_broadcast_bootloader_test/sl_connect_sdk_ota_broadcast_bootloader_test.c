...
{
  APP_INFO("(server): get segment, start: %lu, end: %lu, tag: 0x%x\n",
...
void emberAfPluginOtaBootloaderServerImageDistributionCompleteCallback(EmberAfOtaBootloaderStatus status)
{
  ...
  for (i = 0; i < target_list_length; i++) {
    ...
    APP_INFO("Target index %d ID 0x%2X: status 0x%x appStatus 0x%x\n",
                                                              ...
                                                              uint8_t imageTag)
{
  ...
  APP_INFO("new incoming image %s (tag=0x%x)\n",
...
{
  APP_INFO("(client): incoming segment, start: %d, end: %d, tag: 0x%x\n",
                                                                   ...
                                                                   uint32_t imageSize)
{
  if (status == EMBER_OTA_BROADCAST_BOOTLOADER_STATUS_SUCCESS) {
    APP_INFO("Image download COMPLETED tag=0x%x size=%d\n",
             imageTag, imageSize);
  } else {
                                                                     ...
                                                                     uint8_t *applicationStatus)
{
  ...
  if (accept) {
    APP_INFO("bootload request for image with tag 0x%x accepted, will bootload in %d ms\n",
...
void cli_bootloader_broadcast_broadcast_distribute(sl_cli_command_arg_t *arguments)
{
  ...
  EmberAfOtaBootloaderStatus status =
    emberAfPluginOtaBootloaderServerInitiateImageDistribution(image_size,
                                                              ...
                                                              target_list_length);
  if (status == EMBER_OTA_BROADCAST_BOOTLOADER_STATUS_SUCCESS) {
    APP_INFO("image distribution initiated\n");
  } else {
    APP_INFO("image distribution failed 0x%x\n", status);
  }
#else
