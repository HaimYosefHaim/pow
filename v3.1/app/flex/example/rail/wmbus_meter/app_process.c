...
void app_process_action(RAIL_Handle_t rail_handle)
{
  ...
  switch (state) {
    case S_SCHEDULE_TX:
      length = WMBUS_SAMPLE_setupFrame(tx_buffer.fifo, access_number, wmbus_accessibility, 12345, 123, true, true);
    ...
    case S_TX_DONE:
      ...
      if ( wmbus_accessibility == WMBUS_ACCESSIBILITY_LIMITED_ACCESS ) {
...
void send_packet_at(RAIL_Handle_t rail_handle, uint16_t length, uint32_t send_at)
{
  last_tx_length = WMBUS_phy_software(tx_buffer.fifo, length, BUFFER_LENGTH);
