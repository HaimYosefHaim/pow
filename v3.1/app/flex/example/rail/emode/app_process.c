  ...
  switch (app_state) {
    ...
    case S_SET_POWER_LEVEL:
      ...
      if (is_raw) {
        ...
        APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_SetTxPower() result:%d", rail_status);
      } else {
        rail_status = RAIL_SetTxPowerDbm(rail_handle, power_deci_dbm);
        power_raw = RAIL_GetTxPower(rail_handle);
        APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_SetTxPowerDbm() result:%d", rail_status);
      }
...
static void handle_periodic_rx(RAIL_Handle_t rail_handle)
{
  ...
  if (rx_ended) {
    ...
    APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_ScheduleRx() result:%d", rail_status);
  }
}
...
static void handle_periodic_tx(RAIL_Handle_t rail_handle)
{
  ...
  if (init_needed) {
    ...
    APP_INFO("Periodic Tx mode, period=%lu; EM%d (sleep); EM%d (active)\n",
  ...
  if (!packet_sending) {
    ...
    APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_StartScheduledTx() result:%d", rail_status);
    packet_sending = true;
  }
}
