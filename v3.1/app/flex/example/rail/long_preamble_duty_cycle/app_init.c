...
RAIL_Handle_t app_init(void)
{
  ...
  preamble_bit_length = calculate_preamble_bit_length_from_time(bit_rate, &duty_cycle_config);

  rail_status = RAIL_SetTxAltPreambleLength(rail_handle, preamble_bit_length);
  APP_WARNING(rail_status == 0, "RAIL_SetTxAltPreambleLength failed with %d \n", rail_status);
  ...
  // CLI info message
  APP_INFO("Duty Cycle\n");
  APP_INFO("Preamble length %d for bitrate %d b/s with %d us off time\n", preamble_bit_length, bit_rate, duty_cycle_config.delay);
