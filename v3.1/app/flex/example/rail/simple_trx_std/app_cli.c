...
//                                Global Variables
// -----------------------------------------------------------------------------
/// Flag, indicating transmit request (button has pressed / CLI transmit request
/// has occurred)
...
void cli_info(sl_cli_command_arg_t *arguments)
{
  ...
  switch (_SILICON_LABS_EFR32_RADIO_TYPE) {
    case _SILICON_LABS_EFR32_RADIO_DUALBAND:
      APP_INFO("  Radio type:   Dual band\n");
      break;
    case _SILICON_LABS_EFR32_RADIO_2G4HZ:
      APP_INFO("  Radio type:   2P4GHz\n");
      break;
    case _SILICON_LABS_EFR32_RADIO_SUBGHZ:
      APP_INFO("  Radio type:   SubGHz\n");
      break;
    default:
      break;
  }
...
void cli_std(sl_cli_command_arg_t *arguments)
{
  ...
  if (*arg0 == 'b') {
    ...
    APP_INFO("IEEE 802.15.4 2.4GHz standard set.\n");
    cli_requests.std_requested = true;
#else
    APP_INFO("IEEE 802.15.4 is unsupported.\n");
#endif
  }
  ...
  if (*arg0 == 'g') {
    ...
    if (arg_cnt == 2) {
      ...
      APP_INFO("Wrong argument of 802.15.4g: the frequency designator is missing\n");
      return;
    }

    // checks the second parameter
    if (strcmp(arg1, "863") && strcmp(arg1, "915")) {
      APP_INFO("Wrong argument of 802.15.4g: %s\n", arg1);
      return;
    }
    ...
    if (!strcmp(arg1, "863")) {
      ...
      APP_INFO("IEEE 802.15.4g 863MHz standard set.\n");
      cli_requests.std_requested = true;
#else
      APP_INFO("IEEE 802.15.4g 863MHz is unsupported.\n");
#endif
    }
    ...
    if (!strcmp(arg1, "915")) {
      ...
      APP_INFO("IEEE 802.15.4g 915MHz standard set.\n");
      cli_requests.std_requested = true;
#else
      APP_INFO("IEEE 802.15.4g 915MHz is unsupported.\n");
#endif
    }
#else
    APP_INFO("IEEE 802.15.4g is unsupported.\n");
#endif
  }

#elif defined SL_CATALOG_FLEX_BLE_SUPPORT_PRESENT
  (void)arguments;
  APP_INFO("Unsupported command\n");
#else
  (void)arguments;
  APP_INFO("Unsupported command\n");
#endif
...
void cli_dest_addr(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Destination Address: %d\n", arg);
...
void cli_cfg_dw(sl_cli_command_arg_t *arguments)
{
  ...
  if (sl_flex_ieee802154_get_std() != SL_FLEX_IEEE802154_STD_IEEE802154_2P4GHZ) {
    ...
    if ((arg_dw == 1) || (arg_dw == 0)) {
      ...
      APP_INFO("Config - Whitening: %s\n", (arg_dw == 0) ? OFF : ON);
    } else {
      APP_INFO("Wrong parameter\n");
    }
#else
    (void)arguments;
    APP_INFO("Config - Whitening: ON, Unwhitening is not supported on xg1 and xg23.\n");
#endif
  } else {
    (void)arguments;
    APP_INFO("Unsupported command\n");
...
void cli_cfg_crc(sl_cli_command_arg_t *arguments)
{
  ...
  if (sl_flex_ieee802154_get_std() != SL_FLEX_IEEE802154_STD_IEEE802154_2P4GHZ) {
    ...
    if ((arg_crc == 2) || (arg_crc == 4)) {
      ...
      APP_INFO("Config - CRC size: %d\n", arg_crc);
    } else {
      APP_INFO("Wrong parameter\n");
    }
#else
    (void)arguments;
    APP_INFO("Config - CRC size: 2, the 4-bytes CRC is not supported on xg1 and xg23.\n");
#endif
  } else {
    (void)arguments;
    APP_INFO("Unsupported command\n");
