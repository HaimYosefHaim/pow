...
//                                Global Variables
// -----------------------------------------------------------------------------
/// Flag, indicating transmit request (button has pressed
/// / CLI transmit request has occured)
...
static void handle_transmit(RAIL_Handle_t rail_handle)
{
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR,
              "sl_flex_ieee802154_transmission() status: %d", status);
  ...
  APP_WARNING(status == packet_size, "BLE RAIL_WriteTxFifo status: %d", status);
  status = RAIL_StartTx(rail_handle, BLE_CHANNEL, RAIL_TX_OPTIONS_DEFAULT, NULL);
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "BLE RAIL_StartTx status: %d", status);
#else
#endif
}
...
#else
#endif
  APP_WARNING(rail_status == RAIL_STATUS_NO_ERROR, "RAIL_StartRx() result:%d",
              rail_status);
}
...
static void handle_receive(RAIL_Handle_t rail_handle)
{
  ...
  do {
    ...
    if (ble_recv_packet == NULL) {
      APP_WARNING(ble_recv_packet != NULL, "BLE received packet is NULL pointer");
    } else {
      if (rx_requested) {
...
static void printf_ble_packet(const sl_flex_ble_advertising_packet_t *packet)
{
  ...
  uint8_t *packet_byte;
  APP_INFO("BLE Packet: (size = %d) (payload_len = %d) {\n", packet_size, payload_len);
...
{
  uint8_t i = 0;
  APP_INFO("BLE Packet has been received. Payload (%d): ", length);
  for (i = 0; i < length; i++) {
