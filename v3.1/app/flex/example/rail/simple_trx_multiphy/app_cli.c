...
void cli_info(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("  Scan mode:       %s\n", scan_mode ? ON : OFF);
  // Print the actual channel if scan mode is OFF
  if (!scan_mode) {
    APP_INFO("  Current channel: %d\n", rx_active_channel);
  }
}
...
void cli_send_packet(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Send packet request on channel %d\n", send_channel);
}
...
void cli_rx_channel(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Receive packet is enabled on channel: %d\n", rx_active_channel);
}
