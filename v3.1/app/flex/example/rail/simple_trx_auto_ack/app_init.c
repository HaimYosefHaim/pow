...
RAIL_Handle_t app_init(void)
{
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "After RAIL_SetStateTiming() result:%d ", status);
  ...
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "After RAIL_WriteAutoAckFifo() result:%d ", status);
  // Enable Auto-ACK
  status = RAIL_ConfigAutoAck(rail_handle, &autoAckConfig);
  APP_WARNING(status == RAIL_STATUS_NO_ERROR, "After RAIL_ConfigAutoAck() result:%d ", status);

  // CLI info message
  APP_INFO("Simple TRX with Auto-ACK");
