...
static void handle_network_form()
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("network formed, radio_channel: %d PAN ID: 0x%04X\n", parameters.radioChannel, parameters.panId);
  } else {
    APP_INFO("network form unsuccessful, error code: 0x%02X\n", status);
  }
...
static void process_message()
{
  if (message_from_connect[LIGHT_SWITCH_MESSAGE_CONTROL_BYTE] & 0x01) {
    ...
    APP_INFO("Toggle message from node: %d, light is %s\n", incoming_message.source,
