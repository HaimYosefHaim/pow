...
void emberAfIncomingMessageCallback(EmberIncomingMessage *message)
{
  ...
  if (message->endpoint == SL_CONNECT_ECDH_DEFAULT_DATA_ENDPOINT) {
    if ((tx_options & EMBER_OPTIONS_SECURITY_ENABLED) && !(message->options & EMBER_OPTIONS_SECURITY_ENABLED)) {
      APP_INFO("Warning: security is enabled, but the incoming message not encrypted\n");
    }
    APP_INFO("Incoming message from 0x%02x: %.*s\n", message->source, message->length, message->payload);
  }
}
...
void emberAfStackStatusCallback(EmberStatus status)
{
  switch (status) {
    case EMBER_NETWORK_UP:
      APP_INFO("Network is UP\n");
      break;
    case EMBER_NETWORK_DOWN:
      APP_INFO("Network is DOWN\n");
      break;
    default:
      break;
  }
