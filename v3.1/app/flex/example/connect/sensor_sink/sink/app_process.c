...
void emberAfIncomingMessageCallback(EmberIncomingMessage *message)
{
  if ((tx_options & EMBER_OPTIONS_SECURITY_ENABLED)
    ...
    // drop the message if security is required but the message is non-encrypted
    return;
  }
  ...
  APP_INFO(" Temperature: %d.%03dC Humidity: %d.%03d%%\n",
...
void emberAfChildJoinCallback(EmberNodeType nodeType,
                              EmberNodeId nodeId)
{
  APP_INFO("Sensor joined with node ID 0x%04X, node type: 0x%02X\n", nodeId, nodeType);
}
