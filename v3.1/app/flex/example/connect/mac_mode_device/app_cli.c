...
void cli_active_scan(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Start active scanning: channel %d, status=0x%02X\n", channel_to_scan, status);
}
...
void cli_commission(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Node parameters commissioned\n");
  } else {
    APP_INFO("Commissioning failed, status 0x%02X\n", status);
  }
}
...
void cli_energy_scan(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Start energy scanning: channel %d, samples %d, status=0x%02X\n", channel_to_scan, samples, status);
}
...
void cli_form(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Network formed\n");
  } else {
    APP_INFO("Network form failed, status 0x%02X\n", status);
  }
}
...
void cli_join_network(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Started the joining process\n");
  } else {
    APP_INFO("Join network failed, status 0x%02X\n", status);
  }
}
...
void cli_set_permit_join(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Permit join set 0x%02X\n", duration);
  } else {
    APP_INFO("Permit join failed\n");
  }
}
...
void cli_poll(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Poll status 0x%02X", status);
}
...
void cli_purge_indirect(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Purge indirect success\n");
  } else {
    APP_INFO("Purge indirect failed, 0x%02X\n", status);
  }
}
...
void cli_send(sl_cli_command_arg_t *arguments)
{
  ...
  if ((mac_frame_info & 0x000F) == EMBER_MAC_ADDRESS_MODE_SHORT) {
    ...
    if (EUI64_SIZE != hex_length) {
      APP_INFO("Source MAC address set failed, not correct length\n");
      return;
    }
  ...
  if (((mac_frame_info & 0x00F0) >> 4) == EMBER_MAC_ADDRESS_MODE_SHORT) {
    ...
    if (EUI64_SIZE != hex_length) {
      APP_INFO("Destination MAC address set failed, not correct length\n");
      return;
    }
    for (uint8_t i = 0; i < EUI64_SIZE; i++) {
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("MAC frame submitted\n");
  } else {
    APP_INFO("MAC frame submission failed, status=0x%02X\n",
             status);
  }
}
...
void cli_set_beacon_payload(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Set beacon payload: {");
  APP_INFO_BUFFER("%02X", contents, length);
...
void cli_set_security_key(sl_cli_command_arg_t *arguments)
{
  ...
  if (hex_length != EMBER_ENCRYPTION_KEY_SIZE) {
    APP_INFO("Security key set failed, not correct length\n");
    return;
  }
...
void cli_set_options(sl_cli_command_arg_t *arguments)
{
  ...
  APP_INFO("Send options set: 0x%02X\n", tx_options);
}
...
void cli_set_poll_destination(sl_cli_command_arg_t *arguments)
{
  ...
  if (dest_addr.addr.shortAddress == EMBER_NULL_NODE_ID) {
    if (EUI64_SIZE != hex_length) {
      APP_INFO("Poll address set failed, not correct length\n");
      return;
    }
    for (uint8_t i = 0; i < EUI64_SIZE; i++) {
  ...
  if (status == EMBER_SUCCESS) {
    APP_INFO("Poll address set\n");
  } else {
    APP_INFO("Poll address set failed, 0x%02X\n", status);
  }
}
...
static bool check_channel(uint16_t channel)
{
  ...
  if (channel < default_channel) {
    APP_INFO("Channel %d is invalid, the first valid channel is %d!\n", channel, default_channel);
    channel_ok = false;
  }
  return (channel_ok);
}
